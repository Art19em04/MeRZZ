...
class INPUT(ctypes.Structure):
    ...
    _anonymous_=('ii',); _fields_=[('type',DWORD),('ii',_INPUT)]
SendInput=ctypes.windll.user32.SendInput; MapVirtualKey=ctypes.windll.user32.MapVirtualKeyW
VK={'BACK':0x08,'TAB':0x09,'ENTER':0x0D,'SHIFT':0x10,'CTRL':0x11,'ALT':0x12,'PAUSE':0x13,'CAPS':0x14,
    'ESC':0x1B,'SPACE':0x20,'PGUP':0x21,'PGDN':0x22,'END':0x23,'HOME':0x24,'LEFT':0x25,'UP':0x26,'RIGHT':0x27,'DOWN':0x28,
    'INS':0x2D,'DEL':0x2E,'0':0x30,'1':0x31,'2':0x32,'3':0x33,'4':0x34,'5':0x35,'6':0x36,'7':0x37,'8':0x38,'9':0x39,
...
def _parse_combo(combo):
    ...
    for p in parts:
        if p in ('CTRL','SHIFT','ALT','LCTRL','RCTRL','LSHIFT','RSHIFT','LALT','RALT','LWIN','RWIN','WIN'): mods.append(p)
        else: main=p
