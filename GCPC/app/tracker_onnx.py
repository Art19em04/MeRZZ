...
def _letterbox(rgb, out_h, out_w):
    ...
    padx = (out_w-nw)//2; pady=(out_h-nh)//2
...
def _map_to_original(xy_list, meta, assume_norm01=True):
    out=[]; iw,ih=meta["in_w"],meta["in_h"]; ow,oh=meta["out_w"],meta["out_h"]; px,py=meta["pad_x"],meta["pad_y"]; s=meta["scale"]
    for (x,y) in xy_list:
        if assume_norm01: X=x*ow; Y=y*oh
        else: X,Y=x,y
        ox=(X-px)/max(1e-8,s); oy=(Y-py)/max(1e-8,s)
        nx=float(np.clip(ox/max(1,iw),0,1)); ny=float(np.clip(oy/max(1,ih),0,1)); out.append((nx,ny))
    return out
...
class ONNXHandTracker:
    ...
    def _pre(self, rgb):
        lb, meta=_letterbox(rgb, self.in_h, self.in_w); x=lb.astype(np.float32)*self.input_scale
        ...
        if self.is_nchw: x=np.transpose(x,(2,0,1))[None,...]
        else: x=x[None,...]
        return x, meta

    @staticmethod
    def _handed(lm):
        try: return "Right" if lm[4][0] > lm[20][0] else "Left"
        except Exception: return "Unknown"

    def _parse(self, y, meta):
        y=y[0]; lm=None; score=1.0
        if y.ndim==2 and y.shape[0]==1 and y.shape[1] in (63,42):
            has_z=(y.shape[1]==63); flat=y.reshape(-1); pts=[]
            for i in range(21):
                xi=flat[i*(3 if has_z else 2)+0]; yi=flat[i*(3 if has_z else 2)+1]; pts.append((float(xi),float(yi)))
            ...
            pts=[]; vals=[]
            for i in range(21):
                x01,y01,v=_argmax2d(y[0,i]); pts.append((x01,y01)); vals.append(v)
    ...
    def process(self, rgb):
        ...
        x, meta=self._pre(rgb); out=self.sess.run(None,{self.input_name:x}); return self._parse(out, meta)
